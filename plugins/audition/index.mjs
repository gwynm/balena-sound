...
async function heartbeat() {
  const current_state = await spotify.getCurrentPlayback();
  previous_state = await processPlayback({current_state,previous_state});
}
...
async function processPlayback({current_state,previous_state}) {
  ...
  } else if (await isAuditionPlaylist(current_state) && Logic.isSkip({current_state,previous_state})) {
    spotify.removeItemFromPlaylist({ item_uri: previous_state.item_uri, context_uri: current_state.context_uri });
  }
  return current_state;
}

async function isAuditionPlaylist(current_state) {
  if (current_state.context_uri && current_state.context_uri.includes("spotify:playlist:")) {
    const playlist_name = await spotify.getPlaylistName(current_state.context_uri);
    return playlist_name.includes(AUDITION_MARKER);
  }
}
